...
function CreateOptions(){
	...
	if global.inCombat{
		instance_create_depth(BUTTON1,BOTTOMROW,0,objAttack)
		instance_create_depth(BUTTON2,BOTTOMROW,0,objItem)
		instance_create_depth(BUTTON3,BOTTOMROW,0,objPsynergy)
		if array_length(global.players[global.turn].djinn) > 0 {
			instance_create_depth(BUTTON4,BOTTOMROW,0,objDjinni)
		}
		if array_length(global.knownSummons) > 0{
			instance_create_depth(BUTTON5,BOTTOMROW,0,objSummon)
		}
		if array_length(global.players[global.turn].rerolls) > 0 {
			instance_create_depth(BUTTONRIGHT2,BOTTOMROW,0,objReroll)
		}
	}else if !global.inCombat and !global.inTown{
		DeleteButtons()
		instance_destroy(objMonster)
		instance_destroy(objChallenge)
		instance_create_depth(BUTTON1,BOTTOMROW,0,objItem)
		instance_create_depth(BUTTON2,BOTTOMROW,0,objPsynergy)
		if array_length(global.players[global.turn].djinn) > 0 {
			instance_create_depth(BUTTON3,BOTTOMROW,0,objDjinni)
		}
		...
		if (_completed_count >= global.floorRequired) {
			instance_create_depth(BUTTONRIGHT2, BOTTOMROW, 0, objContinue)
		}
		...
		if (_completed_count == 0 && array_length(global.dungeonlist) > global.dungeon) {
			...
			if (_has_unvisited_town) {
				instance_create_depth(BUTTONRIGHT2, BOTTOMROW, 0, objTownButton)
			}
		}
		...
		if (array_length(global.floorChallenges) > 0) {
			...
			for (var _ci = 0; _ci < _num; _ci++) {
				...
				instance_create_depth(_cx, _cy, 0, objChallenge, { challenge_index: _ci })
			}
		}
	}else if global.inTown{
		DeleteButtons()
		instance_create_depth(BUTTON1,BOTTOMROW,0,objShopButton)
		instance_create_depth(BUTTON2,BOTTOMROW,0,objItem)
		instance_create_depth(BUTTON3,BOTTOMROW,0,objPsynergy)
		instance_create_depth(BUTTON4,BOTTOMROW,0,objDjinni)
		instance_create_depth(BUTTONRIGHT2,BOTTOMROW,0,objReturn)
		var _btn_objs = [objFaceButton1,objFaceButton2,objFaceButton3,objFaceButton4]
		for (var _p = 0; _p < array_length(global.players); _p++) {
			var _spr = { image: global.players[_p].portrait, charid: _p, hovertext: "Switch to " + global.players[_p].name }
			instance_create_depth(PORTRAIT1 + (_p * 63), PORTRAITROW, 0, _btn_objs[_p], _spr)
